...
class PrefabProcess(base):
    ...
    def _info_get(self):
        ...
        """
        result = []
        if "linux" in self.prefab.platformtype.platformtypes:
            cmdlinux = 'netstat -lntp'
            _, out, _ = self.prefab.core.run(cmdlinux, showout=False)
    ...
    def kill(self, name, signal=9, exact=False):
        ...
        for pid in self.find(name, exact):
            self.prefab.core.run("kill -s {0} {1} ; true".format(signal, pid), showout=False, replaceArgs=False)
