...
class PrefabDeployerBot(app):
    NAME = ''

    def isInstalled(self):
        return self.prefab.core.file_exists(
            "$JSAPPSDIR/deployer_bot/telegram-bot") and self.prefab.core.file_exists('$JSCFGDIR/deployerbot/config.toml')
    ...
    def link_code(self):
        ...
        self.prefab.core.file_link('$CODEDIR/github/jumpscale/jscockpit/deployer_bot/', '$JSAPPSDIR/deployer_bot')
    ...
    def ays_config(self, ays_conf=None):
        ...
        self.prefab.core.dir_ensure(j.sal.fs.getDirName(AYS_CONFIG_PATH))
        self.prefab.core.file_write(location=AYS_CONFIG_PATH, content=content, replaceArgs=True)
    ...
    def create_config(self, token=None, g8_addresses=None, dns=None, oauth=None):
        ...
        self.prefab.core.createDir('$JSCFGDIR/deployerbot')
        content = j.data.serializer.yaml.dumps(cfg)
        self.prefab.core.file_write('$JSCFGDIR/deployerbot/config.yaml', content)
