...
class PrefabProxyClassic(base):
    ...
    def installFilterProxy(self, port=8124, forward=True):
        ...
        self.prefab.core.file_write("/etc/privoxy/config", CONFIG)
    ...
    def installCacheProxy(self, storagemntpoint="/storage", btrfs=False):
        ...
        if not self.prefab.core.dir_exists(storagemntpoint):
        ...
        forbiddentunnels = """
            ...
            webtrekk\..*
            """
        self.prefab.core.file_write("/etc/polipo/forbiddenTunnels", forbiddentunnels)
        ...
        self.prefab.core.file_write("/etc/polipo/config", CONFIG)

        self.prefab.core.run("killall polipo", die=False)
    ...
    def configureClient(self, addr="", port=8123):
        ...
        if self.prefab.prefab.platformtype.myplatform.startswith("ubuntu"):
            f = self.prefab.core.file_read("/etc/apt/apt.conf", "")
            f += "\n%s\n" % config
            self.prefab.core.file_write("/etc/apt/apt.conf", f)
        else:
