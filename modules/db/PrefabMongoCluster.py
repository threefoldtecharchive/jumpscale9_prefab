...
class MongoInstance(Startable):
    ...
    def __init__(self, prefab, addr=None, private_port=27021, public_port=None,
        ...
        self.dbdir = dbdir
        self.logger.info(prefab, private_port, public_port, type_, replica, configdb, dbdir)

    def _install(self):
        super()._install()
        self.prefab.core.dir_ensure(self.dbdir)
        return self.prefab.apps.mongodb.build(start=False)
    ...
    def _start(self):
        ...
        a = self.prefab.processmanager.ensure(self._gen_service_name(), self._gen_service_cmd())
        return a
