...
class PrefabUser(base):
    ...
    def passwd(self, name, passwd, encrypted_passwd=False):
        ...
        if encrypted_passwd:
            self.prefab.core.sudo("usermod -p '%s' %s" % (encoded_password, name))
        else:
            ...
            # self.prefab.core.sudo("echo %s | openssl base64 -A -d | chpasswd" % (shell_safe(encoded_password)))
            # self.prefab.core.sudo("echo %s | openssl base64 -A -d | chpasswd" % (encoded_password))
            self.prefab.core.run("echo \"%s:%s\" | chpasswd" % (name, passwd))
    ...
    def create(self, name, passwd=None, home=None, uid=None, gid=None, shell=None,
        ...
        if createhome:
            options.append("-m")
        self.prefab.core.sudo("useradd %s '%s'" % (" ".join(options), name))
        if passwd:
    ...
    def check(self, name=None, uid=None, need_passwd=True):
        ...
        need_passwd (Boolean) indicates if password to be included in result or not.
            If set to True it parses 'getent shadow' and needs self.prefab.core.sudo access
        """
        ...
        if d:
            ...
            if need_passwd:
                s = self.prefab.core.sudo("getent shadow | egrep '^%s:' | awk -F':' '{print $2}'" % (results['name']))
                if s:
                    results['passwd'] = s
        if results:
            return results
    ...
    def ensure(self, name, passwd=None, home=None, uid=None, gid=None,
        ...
        if not d:
            ...
            if fullname is not None and d.get("fullname") != fullname:
                options.append("-c '%s'" % fullname)
            if options:
                self.prefab.core.sudo("usermod %s '%s'" % (" ".join(options), name))
            if passwd:
                self.passwd(name=name, passwd=passwd, encrypted_passwd=encrypted_passwd)
        if group is not None:
            self.prefab.group.user_add(group=group, user=name)
    ...
    def remove(self, name, rmhome=None):
        ...
        if rmhome:
            options.append("-r")
        self.prefab.core.sudo("userdel %s '%s'" % (" ".join(options), name))

    def list(self):
        users = self.prefab.core.find("/home", recursive=False)
