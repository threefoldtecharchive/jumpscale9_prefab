...
cmd = "eval `ssh-agent -s`; ssh-add %s;mkdir /tmp/js9_sb;rsync -avzr -e \"ssh -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null\" %s:/opt/var/build/sandbox/python3/ /tmp/js9_sb/" % (
    pub_ssh_key_path[:-4], ipaddr)
print(cmd)
