...
def upload(prefab):
    ...
    """
    prefab.core.execute_bash('''curl -b 'caddyoauth=%s' -F file=@/opt/var/build/sandbox/js_sandbox.tar.gz https://hub.gig.tech/api/flist/me/upload''' % (iyo_client.jwt))
    ...
    sources = ['gig-official-apps/ubuntu1604-for-js.flist',
                ...
                '{}/js_sandbox.flist'.format(zhub_client.config.data['username'])]
    target = 'js_sandbox_full.flist'
    ...
    if resp.status_code != 200:
        raise RuntimeError('Failed to create js sandbox flist. Error: {}'.format(resp.text))
