...
class PrefabExampleMongoCluster(base):
    ...
    def install(self, pubkey=None):
        ...
        kvm1.prefab.development.js8.install()
        kvm2.prefab.development.js8.install()
        ...
        nodes = []
        for i in range(5):
            nodes.append(kvm1.prefab.systemservices.docker.dockerStart(
                "n%s" % i, ports='', pubkey=pubkey, weave=True, ssh=False).executor)
        for i in range(5, 10):
            nodes.append(kvm2.prefab.systemservices.docker.dockerStart(
