...
class CuisineBaseLoader:
    ...
    def __init__(self, executor, prefab):
        ...
        for className in classes:
            # import the class
            exec("from JumpScale.tools.prefab.%s.Cuisine%s import *" % (myClassName, className))
            # attach the class to this class
            do = "self.%s=Cuisine%s(self.executor,self.prefab)" % (className.lower(), className)
            # self.logger.info(do)
            exec(do)
    ...
    def local(self):
        with self._lock:
            if self._local is None:
                from JumpScale.tools.prefab.JSCuisine import JSCuisine
