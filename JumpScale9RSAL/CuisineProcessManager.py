...
class CuisineSystemd(ProcessManagerBase):
    ...
    def ensure(self, name, cmd, env={}, path="", descr="", systemdunit="", autostart=False, wait=0, **kwargs):
        ...
        if not path:
            path = '/root'
        cmd = self._cuisine.core.replace(cmd)
        path = self._cuisine.core.replace(path)
        ...
        envstr = ""
        env.update(self._cuisine.bash.env)
        for name0, value in list(env.items()):
            if value:
                envstr += "Environment=%s=%s\n" % (name0, self._cuisine.core.replace(value))
...
class CuisineTmuxec(ProcessManagerBase):
    ...
    def list(self, prefix=""):
        rc, result, err = self._cuisine.core.run("tmux lsw", profile=True, die=False, showout=False)
        if err:
            return []
    ...
    def ensure(self, name, cmd, env={}, path="", descr="", autostart=False, wait=0):
        ...
        envstr = ""
        for name0, value in list(env.items()):
            envstr += 'export %s="%s" && ' % (name0, self._cuisine.core.replace(value))
        if path:
            cwd = "cd %s &&" % path
            cmd = "%s %s" % (cwd, cmd)
        if envstr != "":
