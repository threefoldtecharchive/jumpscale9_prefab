...
class CuisineAlba(base):
    ...
    def _install_deps_opam(self):
        ...
        # profile fix
        if not self.cuisine.core.file_exists('/root/.profile_js'):
            self.cuisine.core.file_write('/root/.profile_js', "")
        ...
        self.cuisine.core.run('bash $TMPDIR/opam_installer.sh $BINDIR %s' %
        ...
        self.cuisine.core.run(cmd, profile=True, shell=True)

        janestreet = self.cuisine.development.git.pullRepo(
        ...
        self.cuisine.core.run("opam repo --root=%s -k local add janestreet %s || exit 0" %
        ...
        self.cuisine.core.run(cmd, die=False, profile=True, shell=True)
    ...
    def _install_deps_arakoon(self):
        aradest = self.cuisine.development.git.pullRepo(
            'https://github.com/openvstorage/arakoon.git', branch="1.9", depth=None, ssh=False)
        pfx = 'cd %s && source $TMPDIR/opam.env' % aradest

        self.cuisine.core.run('%s && git pull && git checkout %s' % (pfx, self.arakoon_version), shell=True)
        self.cuisine.core.run('%s && make' % pfx, shell=True)

        if self.cuisine.core.file_exists('$TMPDIR/OPAM/4.03.0/lib/arakoon_client/META'):
            self.cuisine.core.file_unlink('$TMPDIR/OPAM/4.03.0/lib/arakoon_client/META')
    ...
    def _install_deps_orocksdb(self):
        ...
        if self.cuisine.core.file_exists('/usr/local/lib/librocksdb.so'):
            self.cuisine.core.run('rm -rfv /usr/local/lib/librocksdb.*')

        if self.cuisine.core.dir_exists('/opt/code/github/domsj/orocksdb'):
            self.cuisine.core.dir_remove('/opt/code/github/domsj/orocksdb', True)

        #
        # install
        #
        commit = '26c45963f1f305825785592efb41b50192a07491'
        orodest = self.cuisine.development.git.pullRepo('https://github.com/domsj/orocksdb.git', depth=None, ssh=False)
    ...
    def _install_deps_sources(self):
        self.cuisine.core.file_write('/etc/apt/sources.list.d/wily-universe.list',
    ...
    def _install_deps_ordma(self):
        ...
        #
        commit = 'tags/0.0.2'
        ordmadest = self.cuisine.development.git.pullRepo(
            'https://github.com/toolslive/ordma.git', depth=None, ssh=False)

        self.cuisine.core.run('cd %s && git pull && git fetch --tags && git checkout %s' % (ordmadest, commit))

        pfx = 'cd %s && source $TMPDIR/opam.env' % ordmadest
        self.cuisine.core.run('%s && eval `${opam_env}` && make install' % pfx)
    ...
    def _install_deps_gobjfs(self):
        ...
        gobjfsdest = self.cuisine.development.git.pullRepo(
            'https://github.com/openvstorage/gobjfs.git', depth=None, ssh=False)

        self.cuisine.core.run('cd %s && git pull && git fetch --tags && git checkout %s' % (gobjfsdest, commit))
        self.cuisine.core.run(
            'cd %s && mkdir -p build && cd build && cmake -DCMAKE_BUILD_TYPE=RELWITHDEBINFO ..' % gobjfsdest)
        self.cuisine.core.run('cd %s/build && make && make install' % gobjfsdest)
    ...
    def _build(self):
        repo = self.cuisine.development.git.pullRepo('https://github.com/openvstorage/alba', depth=None, ssh=False)
